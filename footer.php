...
<footer class="site-footer">
    <div class="footer-card">
        <div class="footer-card__top">
            <div class="footer-brand">
                <?php $ese_ftr_logo = ese_latam_logo(); ?>
                <a href="<?php echo esc_url(home_url('/')); ?>" aria-label="<?php echo esc_attr(get_bloginfo('name')); ?>">
                    <img class="footer-brand__logo"
                         src="<?php echo esc_url($ese_ftr_logo['url']); ?>"
                         alt="<?php echo esc_attr(get_bloginfo('name')); ?>"
                         width="<?php echo (int) $ese_ftr_logo['width']; ?>" height="<?php echo (int) $ese_ftr_logo['height']; ?>">
                </a>
                <?php // Textos editables en Personalizar → ESE Latam → Footer
                // (ver ese_latam_footer_campos en inc/customizer.php). ?>
                <?php
                $ese_ftr_texto    = ese_latam_footer_texto('texto');
                $ese_ftr_destacado = ese_latam_footer_texto('texto_destacado');
                ?>
                <?php if ('' !== $ese_ftr_texto || '' !== $ese_ftr_destacado) : ?>
                    <p class="footer-brand__text">
                        <?php echo esc_html($ese_ftr_texto); ?>
                        <?php if ('' !== $ese_ftr_destacado) : ?>
                            <strong><?php echo esc_html($ese_ftr_destacado); ?></strong>
                        <?php endif; ?>
                    </p>
                <?php endif; ?>
            </div>

            <?php // Una columna por menú (Apariencia → Menús): el título de cada
            // una es el NOMBRE de su menú y los links, sus items. Sin menú
            // asignado todavía, cada columna cae a su lista de $ese_footer_cols
            // — ver ese_latam_footer_columna() en inc/menus.php. ?>
            <nav class="footer-links" aria-label="<?php esc_attr_e('Enlaces del footer', 'ese-latam'); ?>">
                <?php foreach (['footer_1', 'footer_2', 'footer_3'] as $ese_i => $ese_footer_loc) : ?>
                    <?php ese_latam_footer_columna($ese_footer_loc, $ese_footer_cols[$ese_i] ?? null); ?>
                <?php endforeach; ?>
            </nav>

            <?php $ese_ftr_news_ph = ese_latam_footer_texto('newsletter_placeholder'); ?>
            <div class="footer-news">
                <div class="footer-news__block">
                    <?php if ('' !== ese_latam_footer_texto('newsletter_titulo')) : ?>
                        <p class="footer-heading"><?php echo esc_html(ese_latam_footer_texto('newsletter_titulo')); ?></p>
                    <?php endif; ?>
                    <form class="footer-news__form" action="#" method="post">
                        <label class="sr-only" for="footer-newsletter-email"><?php echo esc_html('' !== $ese_ftr_news_ph ? $ese_ftr_news_ph : __('Correo electrónico', 'ese-latam')); ?></label>
                        <input id="footer-newsletter-email" type="email" name="email" required
                               placeholder="<?php echo esc_attr($ese_ftr_news_ph); ?>">
                    ...
                    <?php if ('' !== ese_latam_footer_texto('newsletter_nota')) : ?>
                        <p class="footer-news__note">
                            <?php echo esc_html(ese_latam_footer_texto('newsletter_nota')); ?>
                        </p>
                    <?php endif; ?>
                </div>
                ...
                <?php if ([] !== $ese_redes) : ?>
                    <div class="footer-news__block">
                        <?php if ('' !== ese_latam_footer_texto('redes_titulo')) : ?>
                            <p class="footer-heading"><?php echo esc_html(ese_latam_footer_texto('redes_titulo')); ?></p>
                        <?php endif; ?>
    ...
    <div class="footer-legal">
        <p>© <?php echo esc_html(wp_date('Y') . ' ' . ese_latam_footer_texto('copyright')); ?></p>
        <p><?php echo esc_html(ese_latam_footer_texto('tagline')); ?></p>
    </div>
</footer>
