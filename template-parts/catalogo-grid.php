    ...
    <?php else : ?>
        <div class="catalogo-grid">
            <?php while ($ese_catalogo_grid_query->have_posts()) : $ese_catalogo_grid_query->the_post(); ?>
                ...
                // …reencuadrada a tamaño uniforme para la grilla (inc/imagenes-
                // catalogo.php): los renders vienen a escala física y un producto
                // chico se veía diminuto al lado de un contenedor de 4 ruedas.
                $ese_card_img = ese_latam_foto_catalogo(ese_latam_producto_foto(get_the_ID(), $ese_litraje_valor));
                ?>
                <?php // Toda la tarjeta es un solo enlace a la ficha: media, texto y
                // chip van dentro del <a> (el chip pasa a ser decorativo). ?>
            ...
            <nav class="catalogo-pagination" aria-label="<?php esc_attr_e('Paginación de productos', 'ese-latam'); ?>">
                <a class="catalogo-pagination__side catalogo-pagination__side--prev"
                    ...
                    <svg width="6" height="10" viewBox="0 0 6 10" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <path d="M6 1.41 4.42 0 0 5l4.42 5L6 8.59 3.18 5 6 1.41Z" fill="currentColor"/>
                    </svg>
