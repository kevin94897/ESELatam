 ...
 *
 * @param array{kicker?: string, title?: string, quote?: string, desc?: string, firma?: string, firma_sub?: string, product?: string, alt?: string, callouts?: list<array{label: string, pos: string}>} $args
 *   `quote` admite HTML de post; `product` es una URL absoluta; `pos` es una
 *   de: left-top, left-bottom, right. Todo sale de la ficha del sector
 *   (inc/pcf-sectores.php).
 *
 * @package EseLatam
 */
...
// Sin nada que decir no se pinta la sección.
if ('' === trim($ese_c['title']) && '' === trim($ese_c['quote']) && '' === trim($ese_c['desc'])) {
    return;
}
?>
    ...
    <div class="criterio__text">
        ...
        <?php if ('' !== $ese_c['quote']) : ?>
            <blockquote class="criterio__quote" data-reveal="up">
                <p>“<?php echo wp_kses_post($ese_c['quote']); ?>”</p>
            </blockquote>
        <?php endif; ?>
        ...
        <?php if ('' !== $ese_c['firma'] || '' !== $ese_c['firma_sub']) : ?>
        ...
        <div class="criterio__brand" data-reveal="up">
            ...
            <span><?php echo esc_html($ese_c['firma']); ?></span>
        </div>
        <?php endif; ?>
        <?php if ('' !== $ese_c['firma_sub']) : ?>
            <p class="criterio__brand-sub" data-reveal="up" data-reveal-delay="0.1"><?php echo esc_html($ese_c['firma_sub']); ?></p>
        <?php endif; ?>
    </div>

    <?php if ('' !== $ese_c['product']) : ?>
    <div class="criterio__scene" data-reveal="fade">
        <span class="criterio__shadow" aria-hidden="true"></span>
        <img class="criterio__product" src="<?php echo esc_url($ese_c['product']); ?>"
            alt="<?php echo esc_attr($ese_c['alt']); ?>" loading="lazy" decoding="async"
